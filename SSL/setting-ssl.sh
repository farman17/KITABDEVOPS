...
server {
    server_name $nama_domain;
        ...
        location / {
                ...
                proxy_pass          http://localhost:$port_domain;
                proxy_read_timeout  60s;
        # Fix the "It appears that your reverse proxy set up is broken" error.
        # Make sure the domain name is correct
                proxy_redirect      http://localhost:$port_domain http://$nama_domain;
        }
...
#------------------------------------------------>>
#TAMBAHKAN PARAMETER BERIKUT DIBAWAH samping tanda =:
#$.host -----tanpa tanda petik
#$.host$.request_uri ------tanpa tanda petik
# <<----------------------------------------------
